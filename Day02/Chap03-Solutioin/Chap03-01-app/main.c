...
#include<limits.h> // 타입의 초시,최대값 담고있는 헤더파일
...
int main(void) {
	...
	// 변수에 값 대입하기 [assigment(대입) 왼쪽의 변수에 오른쪽 값을 넣는다.
	a = 10;
	b = a;
	c = a + 20;
	...
	printf("변수 b의 값 %d\n", b);
	printf("변수 c의 값 %d\n", c);
	printf("변수 f의 값 %f\n", f);	// 빨간색 밑줄은 무조건 오류, 노색 밑줄은 경고(!) 실행은 가능
	// %f는 소수점 6쨰 자리까지 출력한다는 변환 문자
	printf("변수 d의 값 %.16f\n", d);
	printf("변수 ch의 값 %c\n", ch);
	printf("변수 ch의 값 %d\n", ch);
	printf("변수 ch의 값 %c\n", 97);
	...
	// 정수형
	// char : 크기는 1byte -> 8bit 00000000 -> 00 ~ FF (0 ~ 255 까지 표현) 최대크기 255
	// short ; 2bytes 
	// int(기본, 제일중요!) : 4bytes
	// long : 4bytes
	// long long : 8 bytes
	...
	printf("변수 int 사이즈, %zu byte,최소 %lld, 최대 %lld \n", sizeof(long long), LLONG_MIN, LLONG_MAX);
	...
	printf("변수 unsigned long long 사이즈, %zu byte,최소 %d, 최대 %llu \n", sizeof(unsigned long long), 0, ULLONG_MAX);
