...
int main() {
	...
	res = (a > b); // a가 b보다 크다);	// false =>0, true => 1 //0
	printf("a > b : %d\n", res);
	printf("a >= b : %d\n", (a >= b));
	res = (a < b); // a가 b보다 작다
	printf("a < b : %d\n", res);
	...
	// 콤마 연산자
	// 한번에 여러개 수식을 차례로 나열해야 할떄 사용
	res = (++a, ++b);  // a도 1증가, b 도 1 증가 후 가장 오른쪽에 있는 피연사자값을 res에 대입하라
	printf("%d, %d, %d, %d\n", a, b, res);
	...
	// 비트 연산자, 2진수 변환 후 -은 false, 1은 true로 감안한다
