...
static int read_menu(void);			// 입력한걸 반환하여 보여주는 타입
static void read_line(char* buf, int size);

static void add_contact(void);
static void list_contact(void);
...
static void add_contact(void) {
	...
	// 2. 현재 빈 칸인 contacts[count] 위치에 정보를 입력받음
    // count가 0이면 첫 번째(0번) 방에 저장하게 됨
	printf("이름[최대10자] : ");
	...
	// 3. 저장이 끝났으니 다음 사람을 위해 숫자를 1 증가시킴
	count++;
	puts("추가 완료!");
}
...
static void update_contact(void) {
	...
	// 이전 버퍼의 데이터를 삭제할 수 없음
	while ((ch = getchar()) != '\n' && ch != EOF) {}	// 첫번쨰 이름을 수정할 수 없음
	...
	if (buf[0] != '\0') {	// 엔터만 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].name[NAME_LEN - 1] = '\0';	// 안전용 추가. 글자를 max 적으면 \0 들어가야 문자열 끝을 알려줌
	}
	...
	if (buf[0] != '\0') {	// 엔터만 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].phone[PHONE_LEN - 1] = '\0';	// 안전용 추가
	}
	...
	if (buf[0] != '\0') {	// 엔터만 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].address[ADDR_LEN - 1] = '\0';	// 안전용 추가
	}
	...
	if (buf[0] != '\0') {	// 엔터만 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].email[EMAIL_LEN - 1] = '\0';	// 안전용 추가
	}
	...
	if (buf[0] != '\0') {	// 엔터만 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].memo[MEMO_LEN - 1] = '\0';	// 안전용 추가
	}
...
static void delete_contact(void) {
	...
	// 2차 작성
	printf("정말 삭제하겠습니까? (y/n) >");
	read_line(yn, (int)sizeof(yn));

	if (!(yn[0] == 'y' || yn[0] == 'Y')) {	// 소문자 y이거나 대문자 Y아니면
		puts("삭제 취소합니다.");
		return;
	}
